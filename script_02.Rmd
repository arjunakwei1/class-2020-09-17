...
```{r}
raw <- readxl::read_xlsx(path="raw_data/class_enrollment_summary_by_term_9-17-2020 (1).xlsx", skip=3)
en_fall_2020 <- raw %>%
  clean_names() %>%
  filter(u_grad>=10)%>%
  ...
  arrange(desc(u_grad)) 
en_fall_2020
gov_50_enrollment <- raw %>%
  clean_names() %>%
  filter(u_grad>=10)%>%
  select(course_id, course_title, course_name, course_department, course_section_code, u_grad) %>%
  drop_na() %>%
  arrange(desc(u_grad)) %>%
...
en_fall_2020 %>%
  ...
  ggplot(aes(x=fct_reorder(course_title, u_grad), y=u_grad))+
    geom_col()+
  coord_flip()+
  labs(title="Enrollment of 10 Largest Harvard Classes", x= "Enrollment", y= "Course Name")
```
